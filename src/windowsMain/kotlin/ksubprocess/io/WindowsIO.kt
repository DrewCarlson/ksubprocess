...
internal fun read(handle: HANDLE?, destination: Memory, offset: Int, length: Int): ssize_t {
    val size = minOf(
    ...
    val result = memScoped {
        ...
        if (ReadFile(handle, destination.pointer + offset, size, rVar.ptr, null) == 0) {
            val ec = GetLastError()
...
internal fun write(handle: HANDLE?, destination: Memory, offset: Int, length: Int): ssize_t {
    val result = memScoped {
        val rVar = alloc<DWORDVar>()
        if (WriteFile(handle, destination.pointer + offset, length.convert(), rVar.ptr, null) == 0) {
            throw IOException(
                "IO operation failed due to windows error",

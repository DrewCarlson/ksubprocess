...
public sealed class PosixException(public val errno: Int, message: String) : Exception(message) {
    ...
    public companion object {
        ...
        public fun forErrno(
            ...
            val posixErrorCodeMessage = when (val posixConstantName = knownPosixErrors[errno]) {
                null -> "POSIX error $errno"
                else -> "$posixConstantName ($errno)"
            }

            val message = "$function failed, $posixErrorCodeMessage: ${posixErrorToString(errno)}"
